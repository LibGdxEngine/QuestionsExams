...
def preview_questions(request, upload_id):
    ...
    languages = questions.values_list("language", flat=True).distinct()
    specificities = questions.values_list("specificity", flat=True).distinct()
    levels = questions.values_list("level", flat=True).distinct()
