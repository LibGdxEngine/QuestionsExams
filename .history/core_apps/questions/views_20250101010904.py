...
from .models import QuestionAnswer, AnswerImage, TempQuestion, Question
import logging
import pandas as pd
    ...
    if request.method == "POST" and request.FILES["file"]:
        ...
        for _, row in df.iterrows():
            TempQuestion.objects.create(
                ...
                level=row["Level"],
                answers=row["Answers"],  # Assuming 'Answers' is the column name
            )

        return JsonResponse(
            {"success": True, "message": "File uploaded and data parsed successfully."}
        )
...
def preview_questions(request):
    temp_questions = TempQuestion.objects.all()
    return render(request, "preview_questions.html", {"questions": temp_questions})
...
def confirm_question(request, question_id):
    if request.method == "POST":
        ...
        temp_question.answers = request.POST.get("answers")
        ...
        return JsonResponse(
            {"success": True, "message": "Question confirmed and saved."}
        )
