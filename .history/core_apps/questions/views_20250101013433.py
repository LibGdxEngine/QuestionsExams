    ...
    if request.method == "POST" and request.FILES["file"]:
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    # Create question with dynamic field mapping
                    question_data = {
                        "text": row.get("text")
                        or row.get("Text")
                        ...
                        "specificity": row.get("specificity") or row.get("Specificity"),
                        "level": row.get("level") or row.get("Level"),
                    }

                    # Remove None values
                    question_data = {
                        k: v for k, v in question_data.items() if v is not None
                    }
                    ...
                    if answers:
                        ...
                        for answer in answer_list:
                            TempAnswer.objects.create(
                                question=question, text=answer.strip()
                            )
