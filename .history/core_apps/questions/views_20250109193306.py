...
def upload_summary(request, upload_id):
    ...
    questions_without_answers = questions.annotate(answer_count=Count("temp_answers")).filter(answer_count=0).count()
    questions_with_correct_answers = questions.filter(temp_answers__is_correct=True).distinct().count()

    language_stats = dict(questions.values("language").annotate(count=Count("id")).values_list("language", "count"))
    specificity_stats = dict(questions.values("specificity").annotate(count=Count("id")).values_list("specificity", "count"))
    level_stats = dict(questions.values("level").annotate(count=Count("id")).values_list("level", "count"))
    year_stats = dict(questions.values("years").annotate(count=Count("id")).values_list("years", "count"))
    subject_stats = dict(questions.values("subjects").annotate(count=Count("id")).values_list("subjects", "count"))

    context = {
        "excel_upload": {
            "uploaded_at": excel_upload.uploaded_at.strftime('%Y-%m-%d %H:%M:%S'),
            "id": excel_upload.id
        },
    ...
    return JsonResponse({'upload_complete': True, 'context': context})
