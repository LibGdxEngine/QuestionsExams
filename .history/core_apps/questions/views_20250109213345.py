...
def preview_questions(request, upload_id):
    ...
    context = {
        "excel_upload": excel_upload,
        "questions": page_obj,
        "questions_with_answers_count": questions.filter(
            temp_answers__isnull=False
        ).count(),
        "questions_with_language_count": questions.filter(
            language__isnull=False
        ).count(),
        "questions_with_year_count": questions.filter(years__isnull=False).count(),
        "questions_with_specificity_count": questions.filter(
            specificity__isnull=False
        ).count(),
...
def save_questions(request):
    if request.method == "POST":
        ...
        logger.debug(f"Initial questions count: {questions.count()}")
        
        if with_answers:
            questions = questions.filter(temp_answers__isnull=False)
            logger.debug(f"Questions with answers count: {questions.count()}")
        if with_language:
            questions = questions.filter(language__isnull=False)
            logger.debug(f"Questions with language count: {questions.count()}")
        if with_year:
            questions = questions.filter(years__isnull=False)
            logger.debug(f"Questions with year count: {questions.count()}")
        if with_specificity:
            questions = questions.filter(specificity__isnull=False)
            logger.debug(f"Questions with specificity count: {questions.count()}")
        if with_level:
            questions = questions.filter(level__isnull=False)
            logger.debug(f"Questions with level count: {questions.count()}")
        if with_system:
            questions = questions.filter(systems__isnull=False)
            logger.debug(f"Questions with system count: {questions.count()}")
        if with_topic:
            questions = questions.filter(topics__isnull=False)
            logger.debug(f"Questions with topic count: {questions.count()}")
