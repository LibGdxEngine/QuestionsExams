...
def preview_questions(request, excel_upload_id):
    ...
    questions_with_answers_count = (
        questions.filter(temp_answers__isnull=False).distinct().count()
    )
    questions_with_language_count = (
        questions.filter(language__isnull=False).distinct().count()
    )
    questions_with_year_count = questions.filter(years__isnull=False).distinct().count()
    questions_with_specificity_count = (
        questions.filter(specificity__isnull=False).distinct().count()
    )
    questions_with_level_count = (
        questions.filter(level__isnull=False).distinct().count()
    )
    questions_with_system_count = (
        questions.filter(systems__isnull=False).distinct().count()
    )
    questions_with_topic_count = (
        questions.filter(topics__isnull=False).distinct().count()
    )
