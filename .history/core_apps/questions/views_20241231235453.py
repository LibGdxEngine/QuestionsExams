...
from .models import QuestionAnswer, AnswerImage
...
def add_answer(request):
    if request.method == 'POST':
        ...
        question_id = request.POST.get('question_id')  # Ensure you pass the question ID
        try:
            question = Question.objects.get(id=question_id)
            # Create the QuestionAnswer instance
            question_answer = QuestionAnswer.objects.create(question=question, answer_text=answer_text)
            # Create AnswerImage instances for each uploaded image
            for image in images:
