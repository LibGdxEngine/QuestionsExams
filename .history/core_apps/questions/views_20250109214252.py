...
def preview_questions(request, excel_upload_id):
    excel_upload = get_object_or_404(ExcelUpload, id=excel_upload_id)
    questions = excel_upload.temp_questions.all()
    ...
    if selected_language:
        questions = questions.filter(language=selected_language)
    if selected_specificity:
        questions = questions.filter(specificity=selected_specificity)
    if selected_level:
    ...
    questions_with_language_count = questions.filter(language__isnull=False).distinct().count()
    questions_with_year_count = questions.filter(years__isnull=False).distinct().count()
    questions_with_specificity_count = questions.filter(specificity__isnull=False).distinct().count()
    questions_with_level_count = questions.filter(level__isnull=False).distinct().count()
    questions_with_system_count = questions.filter(systems__isnull=False).distinct().count()
    questions_with_topic_count = questions.filter(topics__isnull=False).distinct().count()
    ...
    context = {
        ...
        'questions_with_language_count': questions_with_language_count,
        'questions_with_year_count': questions_with_year_count,
        'questions_with_specificity_count': questions_with_specificity_count,
        'questions_with_level_count': questions_with_level_count,
        'questions_with_system_count': questions_with_system_count,
        'questions_with_topic_count': questions_with_topic_count,
...
def save_questions(request):
    if request.method == "POST":
        ...
        excel_upload_id = request.POST.get("excel_upload_id")
        ...
        # Get the ExcelUpload instance
        excel_upload = get_object_or_404(ExcelUpload, id=excel_upload_id)

        # Filter questions based on the ExcelUpload and options
        questions = excel_upload.temp_questions.all()
        if with_answers:
            questions = questions.filter(temp_answers__isnull=False).distinct()
        if with_language:
