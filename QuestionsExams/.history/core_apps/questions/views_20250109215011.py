...
def preview_questions(request, upload_id):
    ...
    if selected_language:
        questions = questions.exclude(language__isnull=True).exclude(language='nan').filter(language=selected_language)
    if selected_specificity:
        questions = questions.exclude(specificity__isnull=True).exclude(specificity='nan').filter(specificity=selected_specificity)
    if selected_level:
        questions = questions.exclude(level__isnull=True).exclude(level='nan').filter(level=selected_level)
    if has_answers == 'yes':
    ...
    languages = questions.exclude(language__isnull=True).exclude(language='nan').values_list('language', flat=True).distinct()
    specificities = questions.exclude(specificity__isnull=True).exclude(specificity='nan').values_list('specificity', flat=True).distinct()
    levels = questions.exclude(level__isnull=True).exclude(level='nan').values_list('level', flat=True).distinct()
    ...
    context = {
        ...
        'questions_with_answers_count': questions.filter(temp_answers__isnull=False).distinct().count(),
        'questions_with_language_count': questions.exclude(language__isnull=True).exclude(language='nan').count(),
        'questions_with_year_count': questions.exclude(years__isnull=True).exclude(years='nan').count(),
        'questions_with_specificity_count': questions.exclude(specificity__isnull=True).exclude(specificity='nan').count(),
        'questions_with_level_count': questions.exclude(level__isnull=True).exclude(level='nan').count(),
        'questions_with_system_count': questions.exclude(systems__isnull=True).exclude(systems='nan').count(),
        'questions_with_topic_count': questions.exclude(topics__isnull=True).exclude(topics='nan').count(),
    }
