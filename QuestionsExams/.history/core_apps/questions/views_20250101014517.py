...
def upload_summary(request, upload_id):
    ...
    total_answers = TempAnswer.objects.filter(question__excel_upload=excel_upload).count()
    questions_without_answers = questions.annotate(
        answer_count=Count('temp_answers')
    ).filter(answer_count=0).count()
    
    questions_with_correct_answers = questions.filter(
        temp_answers__is_correct=True
    ).distinct().count()

    # Get unique values and their counts
    language_stats = Counter(questions.values_list('language', flat=True))
    specificity_stats = Counter(questions.values_list('specificity', flat=True))
    level_stats = Counter(questions.values_list('level', flat=True))
    year_stats = Counter(questions.values_list('years', flat=True))
    subject_stats = Counter(questions.values_list('subjects', flat=True))
    ...
    context = {
        ...
        'unique_languages': set(language_stats.keys()),
        'unique_specificities': set(specificity_stats.keys()),
        'unique_levels': set(level_stats.keys()),
        'unique_years': set(year_stats.keys()),
        'unique_subjects': set(subject_stats.keys()),
...
def preview_questions(request, upload_id):
    ...
    context = {
        ...
        'languages': excel_upload.temp_questions.values_list('language', flat=True).distinct(),
        'specificities': excel_upload.temp_questions.values_list('specificity', flat=True).distinct(),
        'levels': excel_upload.temp_questions.values_list('level', flat=True).distinct(),
