...
from .models import QuestionAnswer, AnswerImage, TempQuestion, Question, Answer, TempAnswer
import logging
import pandas as pd
...
def upload_excel(request):
    if request.method == "POST" and request.FILES["file"]:
        ...
        for _, row in df.iterrows():
            ...
            # Assuming 'Answers' is a CSV string of answers
            answers = row["Answers"].split(",")
            for answer in answers:
                TempAnswer.objects.create(
                    question=question,
                    text=answer.strip(),
                    # Set is_correct based on your logic
                )
