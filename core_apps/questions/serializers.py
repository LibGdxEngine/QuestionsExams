...
class ExamJourneyDetailsSerializerV2(serializers.ModelSerializer):
    ...
    def get_progress(self, obj):
        ...
        # to ensure it contains 'is_disabled', 'is_correct', etc. properly formatted.
        
        # Re-use logic similar to ExamJourneySerializer.get_progress but keep it as a dict.
        processed_progress = {}
        ...
        for key, value in progress_dict.items():
            ...
            # We will skip the text lookup here to avoid DB hits unless we really need it, 
            # or if we want to strictly follow v1 behavior. 
            # Let's assume 'value' has the data usually.
            
            if question_id is None:
                 # If we can't determine an ID, we probably shouldn't return it in the dict keyed by ID.
                 # or we skip.
                 continue
            ...
            processed_progress[str_key] = {
                ...
                "is_disabled": True, # Frontend checks this
            }
